...
def batch_preprocess(batch, pad_idx, eos_idx, reverse=False):
    batch_ns, batch_s = batch
    diff = batch_ns.size(1) - batch_s.size(1)
    if diff < 0:
        pad = torch.full_like(batch_s[:, :-diff], pad_idx)
        batch_ns = torch.cat((batch_ns, pad), 1)
    elif diff > 0:
        pad = torch.full_like(batch_ns[:, :diff], pad_idx)
        batch_s = torch.cat((batch_s, pad), 1)

    ns_styles = torch.ones_like(batch_ns[:, 0])
    s_styles = torch.zeros_like(batch_s[:, 0])

    if reverse:
        batch_ns, batch_s = batch_s, batch_ns
        ns_styles, s_styles = s_styles, ns_styles
        
    tokens = torch.cat((batch_ns, batch_s), 0)
    lengths = get_lengths(tokens, eos_idx)
    styles = torch.cat((ns_styles, s_styles), 0)
...
def train(config, vocab, model_F, model_D, train_iters, dev_iters, test_iters):
    ...
    while True:
        ...
        if global_step % config.eval_steps == 0:
            ...
            valid_eval(config, vocab, model_F, model_D, global_step, dev_iters, temperature, drop_decay, optimizer_D, optimizer_F)
            #auto_eval(config, vocab, model_F, test_iters, global_step, temperature)

def auto_eval(config, vocab, model_F, test_iters, temperature, step):
    model_F.eval()
    vocab_size = len(vocab)
    eos_idx = vocab.stoi['<eos>']
    ...
    ns_iter = test_iters.ns_iter
    s_iter = test_iters.s_iter
    
    gold_text, raw_output, rev_output = zip(inference(ns_iter, 0), inference(s_iter, 1))
    
    evaluator = Evaluator()
    #ref_text = evaluator.sarc_ref

    
    # acc_s = evaluator.sarc_acc_0(rev_output[0])
    # acc_ns = evaluator.sarc_acc_1(rev_output[1])
    #bleu_s = evaluator.sarc_ref_bleu_0(rev_output[0])
    #bleu_ns = evaluator.sarc_ref_bleu_1(rev_output[1])

    for k in range(5):
        idx = np.random.randint(len(rev_output[0]))
        print('*' * 20, 's sample', '*' * 20)
    ...
    print('*' * 20, '********', '*' * 20)
    '''
    print(('[auto_eval] bleu_ns: {:.4f} bleu_s: {:.4f}\n').format(
            bleu_ns, bleu_s
    ))
    ...
    '''
    eval_ns_s = config.save_path + f'/{step}_ns_to_s.txt'
    eval_s_ns = config.save_path + f'/{step}_s_to_ns.txt'

    with open(eval_ns_s, 'w') as f:
        for i in rev_output[1]:
            f.write(f'{i}\n')

    with open(eval_s_ns, 'w') as f:
        for i in rev_output[1]:
            f.write(f'{i}\n')

    '''
    with open(eval_log_file, 'a') as fl:
        print(('iter{:5d}: bleu_ns: {:.4f} bleu_s: {:.4f}\n').format(
            global_step, bleu_ns, bleu_s
        ), file=fl)
...
def d_step_eval(config, vocab, model_F, model_D, optimizer_D, batch, temperature):
    model_F.eval()
    pad_idx = vocab.stoi['<pad>']
    eos_idx = vocab.stoi['<eos>']
    vocab_size = len(vocab)
...
def f_step_eval(config, vocab, model_F, model_D, optimizer_F, batch, temperature, drop_decay,
           cyc_rec_enable=True):
    model_D.eval()
...
def valid_eval(config, vocab, model_F, model_D, global_step, dev_iters, temperature, drop_decay, optimizer_D, optimizer_F):
    his_d_adv_loss = []
    his_f_slf_loss = []
    his_f_cyc_loss = []
    his_f_adv_loss = []
    ...
    for i, batch in enumerate(dev_iters):
        d_adv_loss = d_step_eval(
                config, vocab, model_F, model_D, optimizer_D, batch, temperature
            )
    
        his_d_adv_loss.append(d_adv_loss)

        f_slf_loss, f_cyc_loss, f_adv_loss = f_step_eval(
                config, vocab, model_F, model_D, optimizer_F, batch, temperature, drop_decay
            )
        his_f_slf_loss.append(f_slf_loss)
        his_f_cyc_loss.append(f_cyc_loss)
